            ...
            Route::group(['prefix' => $scope, 'middleware' => "Routeserver:$scope"], function () use ($scope) {
                Route::group(['prefix' => 'paymentstate'], function () use ($scope) {
                    if ($scope == 'admin') {
                        ...
                        Route::get('paymenttransaction', 'PaymentStateController@transaction')->name($scope . '.email.paymentstate.transaction');
                        Route::get('paymentbitcoin', 'PaymentStateController@bitcoin')->name($scope . '.email.paymentstate.bitcoin');
                    }
                });
            });
    }
