...
<link rel="stylesheet" type="text/css" href="https://cdn.datatables.net/1.10.20/css/jquery.dataTables.min.css">
    <div class="table-responsive">
        <div class="btn-area" style="position: absolute;right: 0; margin-right: 300px;z-index: 100;">
            {{--<button class="btn btn-success transaction">Transation</button>--}}
            {{--<button class="btn btn-success btn_paid">Bitcoin</button>--}}
        ...
        <table style="margin-top: 10px; margin-bottom: 10px" id="example" class="table table-striped table-bordered table-hover dataTable dtr-inline reporttable" role="grid"
            ...
            @foreach($transactions as $transaction)
                <tr>
                    <td align="center"></td>
                    <td>{{ $transaction['user']->customer_id }}<input class="myid" type="text" style="display: none;" value="{{ $transaction['user']->customer_id }}" name="">  </td>
                    <td> {{ $transaction['user']->username }} </td>
                    <td> {{ $transaction['user']->metaData->firstname }} </td>
                    <td> {{ $transaction['user']->metaData->lastname }} </td>
                    <td> {{ ($transaction['transaction']->gateway == $transfer_gateway)?'Transfer Wise':'B2B Pay' }}</td>
                    <td> {{ $transaction['transaction']->context }}</td>
                    <td> {{ $transaction['transaction']->amount }} </td>
                    <td> <?php echo $create = date('Y-m-d',strtotime($transaction['transaction']->created_at)); ?>
                    ...
                    <td class="btn-area"> 
                        ...
                        <button class="btn btn-primary pre-delete" data-toggle="modal" data-target="#myModal"  style="display: {{(in_array($transaction['transaction']->id,$transaction_ids))?'':'none !important'}}">Mark as Paid</button>
                       
                        <button class="btn btn-success"  style="display: {{(in_array($transaction['transaction']->id,$transaction_ids))?'none !important':''}}">
    ...
    $(function () {
        ...
        $('.paginationWrapper .pagination li a').click(function (e) {
         ...
         $('.delete').click(function(){
            var self = this;
            $('.'+$(self).attr('attr-id')).html('Sending...');
            $.get('{{ scopeRoute("email.paymentstate.delete") }}', {data:$(this).attr('attr-id')}, function (response){
                $('.'+$(self).attr('attr-id')).attr('style', 'display:none !important');
                $('.'+$(self).attr('attr-id')).parent().children().eq(1).attr('style', 'display:block !important');
            });            
        })
         ...
         $('.transaction').click(function(){
            $.get('{{ scopeRoute("email.paymentstate.transaction") }}', {data:"transaction"}, function (response){
                response = JSON.parse(response);
                var html = '';
                for(let item in response)
                {
         ...
         $('.bitcoin').click(function(){
            $.get('{{ scopeRoute("email.paymentstate.bitcoin") }}', {data:"bitcoin"}, function (response){
                response = JSON.parse(response);
                var html = '';
                for(let item in response)
                {
