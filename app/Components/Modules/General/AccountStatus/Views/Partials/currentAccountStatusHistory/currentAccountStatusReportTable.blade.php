@if($accountStatusData->count())
    ...
    <table class="table table-striped table-bordered table-hover dataTable dtr-inline reporttable" role="grid"
        ...
        <tr>
            ...
            <th> {{ _mt($moduleId,'AccountStatus.new_payment_due') }}</th>
...
<script type="text/javascript">
    ...
    $('.paginationWrapper .pagination li a').click(function (e) {
        ...
        fetchCurrentAccountStatusReport(route);
    });
    ...
    $('.downloadPdf').click(function () {
        $.post('{{ route('currentAccountStatusReport.download.pdf') }}', $('.filterForm').serialize(), function (response) {
    ...
    $('.downloadExcel').click(function () {
        $.post('{{ route('currentAccountStatusReport.download.excel') }}', $('.filterForm').serialize(), function (response) {
    ...
    $('.downloadCsv').click(function () {
        $.post('{{ route('currentAccountStatusReport.download.csv') }}', $('.filterForm').serialize(), function (response) {
    ...
    $('.printReport').click(function () {
        $.post('{{ route('currentAccountStatusReport.print') }}', $('.filterForm').serialize(), function (response) {
            var HTML = response;
