        ...
        foreach (getScopes()->except('shared') as $scope)
            Route::group(['prefix' => $scope, 'middleware' => "Routeserver:$scope"], function () use ($scope) {
                Route::group(['prefix' => 'manage/account'], function () use ($scope) {
                    if ($scope == 'admin') {
                        Route::post('saveAccountStatus', 'AccountStatusController@saveAccountStatus')->name($scope . '.accountStatus.save');
                        ...
                        Route::get('currentAccountStatus', 'AccountStatusController@currentAccountStatusReportIndex')->name('currentAccountStatusReport');
