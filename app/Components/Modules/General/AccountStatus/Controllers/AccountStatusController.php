...
class AccountStatusController extends Controller
{
    ...
    function saveAccountStatus(Request $request)
    {
        ...
        switch ($request->member_status) {
            case 'active' :
                ...
                break;
            case 'inactive' :
                $accountStatus = ['login' => 0, 'commission' => 1, 'payout' => 1, 'fund_transfer' => 1];
                break;
            case 'terminated' :
            ...
            case 'custom' :
                $accountStatus = [
                    'login' => isset($request->login_status) ? 1 : 0,
                    'payout' => isset($request->payout_status) ? 1 : 0,
                    'commission' => isset($request->commission_status) ? 1 : 0,
                    'fund_transfer' => isset($request->fund_transfer_status) ? 1 : 0,
                ];
                break;
        }

        DB::transaction(function () use ($request, $userId, $accountStatus) {
            foreach ($accountStatus as $key => $value) {
                if ($user = UserPermission::where('user_id', $userId)->where('slug', $key)->first()) {
                    UserPermission::where('user_id', $userId)->where('slug', $key)->update([
                        'title' => $request->member_status,
                        'status' => $value
                    ]);
                } else {
                    UserPermission::create([
                        'user_id' => $userId,
                        'title' => $request->member_status,
                        'slug' => $key, 'status' => $value
                    ]);
                }
            }

            $user = User::find($userId);
            if ($accountStatus['login'] && !$user->first()->status) {
                $user->update(['status' => true]);
            }

            AccountStatusHistory::create([
                'user_id' => $userId,
                'status' => $request->member_status,
     ...
     * @return Factory|View
     */
    function currentAccountStatusReportIndex()
    {
        $scope = session('theScope') ?: 'user';
        $data = [
            'scripts' => [
        ...
        $data['breadcrumbs'] = [
            ...
            _mt($this->module->moduleId, 'AccountStatus.current_account_status_report') => 'currentAccountStatusReport'
        ];
