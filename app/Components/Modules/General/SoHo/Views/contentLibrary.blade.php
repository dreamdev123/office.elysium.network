...
@section('content')
  ...
  <link rel="stylesheet" type="text/css" href="{{ asset('slick/slick-theme.css') }}"/>
					...
					@if (count($fileNames) > 0)
						@foreach($fileNames as $filename)
							<div class="each-panel">
								<img src="{{ asset($filename['src']) }}" data-location="library">
							</div>
						@endforeach
					@endif
				</div>
    ...
    $('.brand-group').change(function(){
      if( $(this).is(":checked") ) {
        var brand = $(this).val();
        window.location.href = '{{ scopeRoute('soho.contentLibrary') }}?brand=' + brand;
      }
    });
    ...
    $('.download-btn').on('click', function() {
    	...
    	window.location.href = '{{ scopeRoute('soho.downloadSohoFile') }}?type=image&brand=' + '{{ $brand }}&location=' + location + '&filename=' + selectedImageName;
    })
