...
@section('content')
	...
	<div class="content-creator-page">
		...
		<div class="newSoho-custom-container">
		    ...
		    <div class="newSoho-custom-container">
				...
				<form action="{{ scopeRoute('soho.uploadVideo') }}" method="POST" enctype="multipart/form-data">
				    @csrf
				...
				@if (isset($networkFileNames) && count($networkFileNames) > 0)
					...
					<div class="col-lg-2 col-md-3 col-sm-4 col-6 img-preview-thumbnail">
						<div class="video-item-wrapper">
							<video controls>
							  <source src="{{ $filename['src'] }}#t=1" type="video/mp4">
							  <source src="movie.ogg" type="video/ogg">
							  Your browser does not support the video tag.
							</video>
						</div>
						<i class="fa fa-trash-o remove-sohovideo" data-brand="network" data-filename="{{ $filename['name'] }}"></i>
					</div>
					@endforeach
				@endif
			</div>
			...
			<div class="row">
				@if (isset($capitalFileNames) && count($capitalFileNames) > 0)
					...
					<div class="col-lg-2 col-md-3 col-sm-4 col-6 img-preview-thumbnail">
						...
						<i class="fa fa-trash-o remove-sohovideo" data-brand="capital" data-filename="{{ $filename['name'] }}"></i>
					</div>
					@endforeach
				@endif
			</div> -->
			...
			<div class="row">
				@if (isset($insiderFileNames) && count($insiderFileNames) > 0)
					...
					<div class="col-lg-2 col-md-3 col-sm-4 col-6 img-preview-thumbnail">
						...
						<i class="fa fa-trash-o remove-sohovideo" data-brand="insider" data-filename="{{ $filename['name'] }}"></i>
					</div>
					@endforeach
				@endif
			</div>
	...
	<script>
	  ...
	  Dropzone.options.documentDropzone = {
	    url: '{{ scopeRoute('soho.uploadVideo') }}',
	    maxFilesize: 200, // MB
    ...
    $('.brand-group').change(function(){
	  ...
	  $('.remove-sohovideo').click(function() {
	  	...
	  	window.location.href = '{{ scopeRoute('soho.removeFile') }}?type=video&brand='+brand+'&filename='+filename;
	  })
	</script>
