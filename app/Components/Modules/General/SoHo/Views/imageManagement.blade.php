...
@section('content')
			...
			<div class="file-upload-dropzone-custom-form">
				...
				<div class="newSoho-custom-container">
					<div class="choose-brand-section" style="padding-top: 5px; padding-bottom: 5px;">
		                ...
		                <input type="radio" class="location-group" name="location" value="samples" data-title="Content Creator" {{ $location == 'samples' ? 'checked' : '' }} />
		                <span class="checkbox-circle"></span>
		                <span class="checkbox-name">Content Creator</span>
			            </label>
			            <label class="radio-container">
		                <input type="radio" class="location-group" name="location" value="library" data-title="Content Library" {{ $location == 'library' ? 'checked' : '' }} />
		                <span class="checkbox-circle"></span>
		                <span class="checkbox-name">Content Library</span>
			            </label>
			          </div>
							</div>
						</div>
				...
				<form action="{{ scopeRoute('soho.uploadImage') }}" method="POST" enctype="multipart/form-data">
				    @csrf
				...
				@if (isset($networkFileNames) && count($networkFileNames) > 0)
					@foreach($networkFileNames as $filename)
						<div class="col-lg-2 col-md-3 col-sm-4 col-6 img-preview-thumbnail">
							<img src="{{ $filename['src'] }}">
							<i class="fa fa-trash-o remove-sohoimage" data-brand="network" data-location="{{ $filename['location'] }}" data-filename="{{ $filename['name'] }}"></i>
						</div>
					@endforeach
				@endif
			</div>
			...
			<div class="row">
				@if (isset($capitalFileNames) && count($capitalFileNames) > 0)
					@foreach($capitalFileNames as $filename)
						<div class="col-lg-2 col-md-3 col-sm-4 col-6 img-preview-thumbnail">
							<img src="{{ $filename['src'] }}">
							<i class="fa fa-trash-o remove-sohoimage" data-brand="capital" data-location="{{ $filename['location'] }}" data-filename="{{ $filename['name'] }}"></i>
						</div>
					@endforeach
				@endif
			</div> -->
			...
			<div class="row">
				@if (isset($insiderFileNames) && count($insiderFileNames) > 0)
					@foreach($insiderFileNames as $filename)
						<div class="col-lg-2 col-md-3 col-sm-4 col-6 img-preview-thumbnail">
							<img src="{{ $filename['src'] }}">
							<i class="fa fa-trash-o remove-sohoimage" data-brand="insider" data-location="{{ $filename['location'] }}" data-filename="{{ $filename['name'] }}"></i>
						</div>
					@endforeach
				@endif
			</div>
	...
	<script>
	  ...
	  Dropzone.options.documentDropzone = {
	    url: '{{ scopeRoute('soho.uploadImage') }}',
	    maxFilesize: 5, // MB
      ...
      var location = $("input[type='radio'].location-group:checked").val();
      let send_data = {};
      send_data['brand'] = brand;
      send_data['location'] = location;
      $.ajax({
        type: 'POST',
        url: '{{ scopeRoute('soho.storeBrand') }}',
        data: send_data,
    ...
    $('.location-group').change(function(){
      if( $(this).is(":checked") ) {
        var val = $(this).val();
        let send_data = {};
	      send_data['location'] = val;
	      $.ajax({
	        type: 'POST',
	        url: '{{ scopeRoute('soho.storeBrand') }}',
	        data: send_data,
	        success: function(data) {
	          window.location.href = '{{ scopeRoute('soho.imageManagement') }}?location=' + val;
	        },
	        error: function(data){
	          console.log(data);
	        }
	  ...
	  $('.remove-sohoimage').click(function() {
	  	var filename = $(this).attr('data-filename');
	  	var brand = $(this).attr('data-brand');
	  	var location = $(this).attr('data-location');
	  	window.location.href = '{{ scopeRoute('soho.removeFile') }}?type=image&brand='+brand+'&location='+location+'&filename='+filename;
	  })
	</script>
