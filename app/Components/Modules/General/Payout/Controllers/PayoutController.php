...
//        ]
     ...
     * @return Factory|View
     */
    function releaseRequests(ModuleServices $moduleServices)
    {
        $data = [];
        $data['moduleId'] = $this->module->moduleId;
        $defaults = collect([
        ...
        $payout_requests = PayoutRequest::whereBetween('created_at', [$defaults->get('fromDate'), $defaults->get('toDate')])
            ->when($request_amount = $defaults->get('minimum'), function ($query) use ($request_amount) {
                /** @var Builder $query */
                $query->where('request_amount', '>=', $request_amount);
            })
            ->when($status = $defaults->get('status'), function ($query) use ($status) {
                /** @var Builder $query */
                $query->where('status', $status);
            })
            ->orderBy($defaults->get('sortBy'), $defaults->get('orderBy'))->get();
        $data['payout_requests'] = $payout_requests;
        $data['targets'] = array($moduleServices->callModule('Payment-TransferWise'), $moduleServices->callModule('Payment-ZotaPay'), $moduleServices->callModule('Payment-IPayWallet'));
...
// daa aa benefit file ethayirunnu ?
        $data = [
            'moduleId' => $this->module->moduleId,
            'scripts' => [
                asset('global/plugins/select2/js/select2.full.min.js')
            ...
            'targets' => array($moduleServices->callModule('Payment-ZotaPay'))
            // 'targets' => array_filter($moduleServices->getPaymentPool(), function ($value) {
            //     return $value instanceof PayoutGatewayInterface;
            // }),
        ];

        $data['countries'] = \App\Eloquents\Country::where('active', 1)->get();
    ...
    function walletPicker(Request $request, ModuleServices $moduleServices, PayoutServices $payoutServices)
    {
        ...
        $data['balance'] = $payoutServices->balanceByWallet(loggedUser(), $walletModule) - Transaction::where('context', 'commission')->where('payee', loggedId())->where('status', 0)->sum('amount');
        $requestDetails = $payoutServices->getPayoutRequests([
            'userId' => loggedId(),
    ...
    function validateRequest(Request $request, Plugins $plugins)
    {
        ...
        $balance = $walletModule->getBalance(loggedUser(), false) - Transaction::where('context', 'commission')->where('payee', loggedId())->where('status', 0)->sum('amount');
...
//        $agreementLevel = $plugins->getLevel(loggedId());
        ...
        $validator->after(function ($validator) use ($transactionCharges, $payoutConfig, $balance, $request) {
            if (round(bcadd($request->input('request_amount'), $transactionCharges, 8), 2) > round($balance, 2))
...
//            }
            $request_amount = (float)$request->input('request_amount');
            if ((round($balance, 2) - round($request_amount, 2)) < 0 || $request_amount <= 0)
                $validator->errors()->add('balance', _mt($this->module->moduleId, 'Payout.Insufficient_balance'));
        });
    ...
    {
        $gateway = (int)$request->input('gateway');
        $user = User::find(loggedId());
        $IPayout_id = \App\Eloquents\Module::where('slug', 'Payment-TransferWise')->first()->id;
        // if (isset($IPayout_id)) {
        ...
        //     }
        // }
        return getModule($gateway)->payoutView();
    }
    ...
    public function releaseRequestedPayouts(Request $request, TransactionServices $transactionServices, PayoutServices $payoutServices)
    {
        ...
        foreach ($releaseArray as $key => $amount) {
            ...
            $statusArray[] = ['request_id' => $key, 'status' => 'success'];
    ...
    public function releaseRequestedRemotePayouts($username, $request_amount)
    {
        $user = User::where('username', $username)->first();

        $wallet_id = \App\Eloquents\Module::where('slug', 'Wallet-Ewallet')->first()->id;
        $gateway_id = \App\Eloquents\Module::where('slug', 'Payment-TransferWise')->first()->id;

        $attributes = [
            'payee' => $user->id,
            'payer' => $user->id,
            'amount' => $request_amount,
            'actual_amount' => $request_amount,
            'context' => 'payout',
            'gateway' => $gateway_id,
            'ip' => request()->ip(),
            'status' => 1,
        ...
        $attr_details = [
            ...
            'remarks' => 'payout',
            'from_module' => $wallet_id,
            'to_module' => $gateway_id,
            'operation_id' => TransactionOperation::slugToId('payout')
        ];
        $transaction_detail = new TransactionDetail($attr_details);
        ...
        $payoutRequest = new PayoutRequest($attr_payout_request);
        $payoutRequest->save();
        ...
        $payoutRequest->update(['status' => $approvedSlug, 'release_amount' => $request_amount]);
        $transaction->update(['status' => 1]);
