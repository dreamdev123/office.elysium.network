    ...
    <div class="portlet-body">
        <div class="userWrapper">
            ...
            <div class="allUsers">
                <form class="filterForm row">
                    ...
                    <div class="filters row">
                        ...
                        <div class="filterButtonWrapper col-md-5">
                            ...
                            @if(isset($payout_requests) && count($payout_requests) > 0)
                            <button type="button" class="btn dark downloadExcel" data-style="contract" style="margin: 30px 0 0 20px;">
                ...
                <table class="table table-bordered table-hover fixed" id="Report_PackageSales" style="width: auto;display: none;">
                    <thead>
                        <tr>
                            <th>No</th>
                            <th>Username</th>
                    ...
                    <tbody>
                        ...
                        @php
                            $user = \App\Eloquents\User::find($payout_request->user_id);
                            if($payout_request->gateway == 110)
                            {
                                $fee = 2.25;
                            }
                            elseif($payout_request->gateway == 99)
                            {
                                $fee = 3.88;
                            }
                            elseif($payout_request->gateway == 106)
                            {
                                $fee = 4.99;
                            }
                            $release = $payout_request->request_amount - $fee + 0.63;
                        @endphp
                        <tr>
                            <td>{{ $loop->iteration }}</td>
                            <td>{{ idToUsername($payout_request->user_id) }}</td>
                            <td>{{ $user->customer_id }}</td>
                            <td>{{ fullName($user) }}</td>
                            @foreach($targets as $target)
                                @if($target->moduleId == $payout_request->gateway)
                                    <td>{{ $target->registry['name'] }}</td>
                                @endif
                            @endforeach
                            <td>{{ $payout_request->content }}</td>
                            <td>{{ $payout_request->reference }}</td>
                            <td>{{ $payout_request->request_amount }}</td>
                        </tr>
                        @endforeach
                    </tbody>
...
<script>
    ...
    $('.downloadExcel').click(function () {
        exportTableToExcel('Report_PackageSales','Payout_request');
    });
