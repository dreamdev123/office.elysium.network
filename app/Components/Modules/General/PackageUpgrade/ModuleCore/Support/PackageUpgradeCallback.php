...
class PackageUpgradeCallback extends Callback
{
    ...
    function success(OrderServices $orderServices, $response, Plugins $plugins, RegistrationCallback $registrationCallback)
    {
        return DB::transaction(function () use ($orderServices, $response, $plugins, $registrationCallback) {
            ...
            //var_dump($orderData['transaction_id']);
            $packageId = $orderData['products'][0]['productId'];
            $transaction = isset($response['transaction_id']) ? Transaction::find($response['transaction_id']) : Transaction::find($response['orderData']['transaction_id']);
            ...
            if($transaction)
            {
                ...
                {
                    $user->update(['package_id' => $packageId,'expiry_date'=>null]);
                }
                else
                {
                    $user->update(['package_id' => $packageId,'expiry_date'=>Carbon::now()->addMonth($package->validity_in_month)->toDateTimeString()]);
                }


                defineAction('postPackageUpgrade', 'package_upgrade', collect(['result' => $response, 'previousPackage' => $previousPackage, 'currentPackage' => $package]));
                ...
                $extenalmailservice->sendupgradeemail($user, $package);
