...
class HoldingTankController extends Controller
{
    ...
    function index(BinaryServices $binaryServices)
    {
        ...
        if (!$moduleData->get('holding_tank') || !$user->holding_tank_active)
            return view('General.HoldingTank.Views.Partials.suspended', [
                'moduleId' => $this->module->moduleId
            ]);
    ...
    function changeStatus(Request $request)
    {
        ...
        $position = $request->input('default_position')?$request->input('default_position'):1;
    ...
    function placeUser($userId, $position = 1)
    {
        ...
        if(!$position || $position == 3 || $position == 4)
        {
            $position = 1;
        }
    ...
    function userStatus(Request $request, BinaryServices $binaryServices)
    {
        $data = [
            'moduleId' => $this->module->moduleId,
            ...
            'position'  => User::where('id', $request->input('userId'))->first()->repoData->position
        ];


        return view('General.HoldingTank.Views.Partials.holdingFilterUser', $data);
    }
}