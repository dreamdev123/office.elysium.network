...
class XoomService
{
    ...
    public static function connect(?XoomUser $xoomUser, Request $request, User $user, UserMeta $userMeta, Collection $moduleData): XoomUser
    {
        ...
        if (!$xoomUser) {
            $del_xoomUser = XoomUser::withTrashed()->where('user_id', loggedId())->first()->id;
            if ($del_xoomUser) {
                $xoomUser = XoomUser::withTrashed()->where('user_id', loggedId())->update(['deleted_at' => null]);
                $xoomUser = XoomUser::where('user_id', loggedId())->first();
            } else {
    ...
    public static function deleteUser(XoomUser $xoomUser, Collection $moduleData): XoomUser
    {
        $userExists = ABAPI::checkIfUserExistsByUserId($xoomUser->xoom_user_id, ABAPI::getApplicationAccessToken($moduleData)->access_token);

        if ($userExists) {
            // delete user
            ABAPI::deleteAccount($xoomUser, $moduleData);
