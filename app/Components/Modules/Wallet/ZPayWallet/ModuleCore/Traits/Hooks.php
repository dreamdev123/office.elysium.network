...
use App\Components\Modules\Wallet\ZPayWallet\ModuleCore\Support\TransferCallback;
...
trait Hooks
{
    ...
    function prePayment(PaymentServices $paymentServices, TransferCallback $transferCallback)
    {
        registerAction('prePaymentProcessAction', function ($request) use ($transferCallback, $paymentServices) {
            /** @var Request $request */
            if ($request->input('wallet') != callModule('Wallet-Ewallet')->moduleId)
                return;
    ...
    function prepareTransferPayable(Request $request, PaymentServices $paymentServices)
    {
        ...
        foreach ($request->input('payee') as $each) {
            $attributes = [
                ...
                'total' => $actualAmount + array_sum(array_column($charges = defineFilter('PreZPayWalletTransaction', [], 'transactionTotal', $each), 'amount')),
