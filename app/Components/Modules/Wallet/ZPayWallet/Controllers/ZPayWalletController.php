...
namespace App\Components\Modules\Wallet\ZPayWallet\Controllers;
...
use App\Components\Modules\Wallet\ZPayWallet\ZPayWalletIndex;
use App\Components\Modules\Wallet\ZPayWallet\ZPayWalletIndex as Module;
use App\Components\Modules\Wallet\ZPayWallet\ModuleCore\Eloquents\ZPayWallet;
use App\Components\Modules\Wallet\ZPayWallet\ModuleCore\Eloquents\User;
use App\Components\Modules\Wallet\ZPayWallet\ModuleCore\Traits\Validations;
...
/**
 * Class ZPayWalletController
 * @package App\Components\Modules\Wallet\ZPayWallet\Controllers
 */
class ZPayWalletController extends Controller
{
    use Validations, GraphHelper, DateTimeFormatter;

    /**
     * @var Application|ZPayWalletIndex
     */
    protected $module;
    ...
    function index()
    {
        $data = [
            ...
            'title' => _mt($this->module->moduleId, 'ZPayWallet.ZPayWallet-Wallet'),
            'heading_text' => _mt($this->module->moduleId, 'ZPayWallet.ZPayWallet-Wallet'),
            'breadcrumbs' => [
                _t('index.home') => 'admin.home',
                _mt($this->module->moduleId, 'ZPayWallet.Wallets') => getScope() . '.ZPayWallet',
                _mt($this->module->moduleId, 'ZPayWallet.ZPayWallet-Wallet') => getScope() . '.ZPayWallet'
            ],
            'wallet' => [],
        ...
        return view('Wallet.ZPayWallet.Views.index', $data);
    }
    ...
    function requestUnit(Request $request,ModuleServices $moduleServices)
    {
        ...
        if($unitMethod == 'sync')
        {
            ...
            $balance = $moduleServices->callModule('Payment-ZotaPay')->createuser($user);
        ...
        return response()->json(['status' => false, 'message' => _mt($this->module->moduleId, 'ZPayWallet.The_action_is_not_allowed')]);
    }
    ...
    function getOverView()
    {
        ...
        return view('Wallet.ZPayWallet.Views.Partials.overView', $data);
    }
    ...
    function getIncomeChart()
    {
        ...
        return view('Wallet.ZPayWallet.Views.Partials.incomeExpenseGraph', $data);
    }
    ...
    function getVcc()
    {
        ...
        return view('Wallet.ZPayWallet.Views.Partials.vcc', $data);
    }
    ...
    function getTransfer(Request $request, ModuleServices $moduleServices)
    {
        $data = [
            ...
            'moduleId' => $this->module->moduleId,
            'gateway' => callModule('Payment-ZotaPay')->moduleId,
            'walletConfig' => getModule($this->module->moduleId)->getModuleData(true),
        ];
        $walletData = ZPayWallet::where('user_id', loggedId())->first();

        if (!checkAccess(loggedId(), 'fund_transfer'))
            return view('Common.Partial.no_access', ['message' => _mt($this->module->moduleId, 'ZPayWallet.Sorry_but_you_don_have_permission_to_send_fund')]);
        else if ($walletData->ip_status && $walletData->ip != $request->ip())
            return view('Common.Partial.no_access', ['message' => _mt($this->module->moduleId, 'ZPayWallet.Sorry_but_you_don_have_permission_to_access_this_page')]);
        else
            return view('Wallet.ZPayWallet.Views.Partials.transfer', $data);
    }
    ...
    {
        return $moduleServices->callModule('Payment-ZotaPay')->renderView();
    }
    ...
    function hasSufficientBalance(Request $request)
    {
        ...
        foreach ($request->input('payee.*') as $id => $payee) {
            ...
            $totalAmount += $payee['amount'] + array_sum(array_column($charges = defineFilter('PrZPayWalletTransaction', [], 'transactionTotal', $payee), 'amount'));
        }
    ...
    function getTransactionList(TransactionServices $transactionServices, Request $request)
    {
        ...
        return view('Wallet.ZPayWallet.Views.Partials.transactionList', $data);
    }
    ...
    function getPayInOutChart()
    {
        ...
        return view('Wallet.ZPayWallet.Views.Partials.payInOutChart', $data);
    }
    ...
    function getSettings()
    {
        $data = [
            'scripts' => [],
            'walletSettings' => ZPayWallet::where('user_id', loggedId())->first(),
            'moduleId' => $this->module->moduleId
        ];

        return view('Wallet.ZPayWallet.Views.Partials.settings', $data);
    }
    ...
    function getIpWhiteList()
    {
        ...
        return view('Wallet.ZPayWallet.Views.Partials.ipWhitelist', $data);
    }
    ...
    function getPasswordChanger()
    {
        ...
        if ($data['walletSettings']->transaction_password)
            return view('Wallet.ZPayWallet.Views.Partials.passwordChange', $data);
        else
            return view('Wallet.ZPayWallet.Views.Partials.setWalletPassword', $data);
    }
    ...
    function changePassword(Request $request)
    {
        ...
        ZPayWallet::where('user_id', loggedId())->update(['transaction_password' => bcrypt($tranPasswordData['password'])]);
        $data = [
    ...
    function whitelist(Request $request)
    {
        ...
        ZPayWallet::where('user_id', loggedId())->update($data);
        $data = [
            'module_id' => $this->module->moduleId
        ];
    ...
    function renderTranPasswordView()
    {
        ...
        return view('Wallet.ZPayWallet.Views.Partials.checkTranPassword', $data);
    }
    ...
    function setTransactionPassword(Request $request)
    {
        ...
        ZPayWallet::where('user_id', loggedId())->update(['transaction_password' => bcrypt($whitelistData['password'])]);
        $data = [
            'module_id' => $this->module->moduleId
        ];
        defineAction('postChangeTransactionPasswordActions', 'change_transaction_password', $data);
    ...
    function renderLoginPasswordView()
    {
        ...
        return view('Wallet.ZPayWallet.Views.Partials.checkLoginPassword', $data);
    }
     ...
     */
    function checkZPayWalletPassword(Request $request)
    {
        if (Gate::denies('ZPayWalletTransaction'))
            return response()->json(['status' => false, 'error' => _mt($this->module->moduleId, 'ZPayWallet.Incorrect_transaction_password')], 401);
    ...
    function getMemberTransactionList(TransactionServices $transactionServices, Request $request, $user = null)
    {
        ...
        return view('Wallet.ZPayWallet.Views.Partials.memberTransactionList', $data);
    }
