<div class="wallet transactionPassWrapper">
    <div class="innerWrap">
        <div class="eachField">
            <label>{{_mt($moduleId,'ZPayWallet.please_enter_your_login_password') }}</label>
        </div>
        <div class="eachField">
            <input type="password" placeholder="{{_mt($moduleId,'ZPayWallet.enter_login_password') }}"
...
<script type="text/javascript">
    ...
    $('body').off('click', '.submitTransactionPass').on('click', '.submitTransactionPass', function () {
        ...
        $.post('{!! route(strtolower(getScope()).".ZPayWallet.set_password") !!}', options, function (response) {
            Ladda.stopAll();
