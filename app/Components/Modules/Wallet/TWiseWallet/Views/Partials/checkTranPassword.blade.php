<div class="wallet transactionPassWrapper">
    <div class="innerWrap">
        <div class="eachField">
            <label>{{_mt($moduleId,'ZPayWallet.please_enter_your_tran_password') }}</label>
        </div>
        <div class="eachField">
            <input type="password" placeholder="{{_mt($moduleId,'ZPayWallet.enter_tran_password') }}"
        ...
        <div class="eachField">
            <button class="btn dark submitTransactionPass" type="button">
                ...
                <span>{{_mt($moduleId,'ZPayWallet.submit') }}</span>
            </button>
        </div>
    </div>
</div>
<script type="text/javascript">
    ...
    $('body').off('click', '.submitTransactionPass').on('click', '.submitTransactionPass', function () {
        ...
        $.post('{{ route(strtolower(getScope()).".ZPayWallet.submitTransactionPass") }}', options, function (response) {
            Ladda.stopAll();
            if (window.hasOwnProperty('transactionPassSuccessCallback')) {
                window.transactionPassSuccessCallback(response, 'ZPayWallet');
            } else {
            ...
            switch (response.status) {
                case 403:
                    $('.ZPayWallet.transactionErrors').empty().append('<div class="error">Session timeout !</div>').slideDown();
                    break;
                case 401:
                    var errors = response.responseJSON;
                    $('.ZPayWallet.transactionErrors').empty().html('<div class="error">' + errors.error + '</div>').slideDown();
                    break;
                case 422:
                    var errors = response.responseJSON;
