...
namespace App\Components\Modules\Wallet\TWiseWallet\Controllers;
...
use App\Components\Modules\Wallet\TWiseWallet\TWiseWalletIndex;
use App\Components\Modules\Wallet\TWiseWallet\TWiseWalletIndex as Module;
...
/**
 * Class TWiseWalletController
 * @package App\Components\Modules\Wallet\TWiseWallet\Controllers
 */
class TWiseWalletController extends Controller
{
    use Validations, GraphHelper, DateTimeFormatter;

    /**
     * @var Application|TWiseWalletIndex
     */
    protected $module;
    ...
    function index()
    {
        $data = [
            ...
            'title' => _mt($this->module->moduleId, 'TWiseWallet.TWiseWallet-Wallet'),
            'heading_text' => _mt($this->module->moduleId, 'TWiseWallet.TWiseWallet-Wallet'),
            'breadcrumbs' => [
                _t('index.home') => 'admin.home',
                _mt($this->module->moduleId, 'TWiseWallet.Wallets') => getScope() . '.TWiseWallet',
                _mt($this->module->moduleId, 'TWiseWallet.TWiseWallet-Wallet') => getScope() . '.TWiseWallet'
            ],
            'wallet' => [],
        ...
        return view('Wallet.TWiseWallet.Views.index', $data);
    }
    ...
    function requestUnit(Request $request,ModuleServices $moduleServices)
    {
        ...
        return response()->json(['status' => false, 'message' => _mt($this->module->moduleId, 'TWiseWallet.The_action_is_not_allowed')]);
    }
    ...
    function getOverView()
    {
        ...
        return view('Wallet.TWiseWallet.Views.Partials.overView', $data);
    }
    ...
    function getIncomeChart()
    {
        ...
        return view('Wallet.TWiseWallet.Views.Partials.incomeExpenseGraph', $data);
    }
    ...
    function getVcc()
    {
        ...
        return view('Wallet.TWiseWallet.Views.Partials.vcc', $data);
    }
    ...
    function getTransfer(Request $request, ModuleServices $moduleServices)
    {
        ...
        $walletData = TWiseWallet::where('user_id', loggedId())->first();

        if (!checkAccess(loggedId(), 'fund_transfer'))
            return view('Common.Partial.no_access', ['message' => _mt($this->module->moduleId, 'TWiseWallet.Sorry_but_you_don_have_permission_to_send_fund')]);
        else if ($walletData->ip_status && $walletData->ip != $request->ip())
            return view('Common.Partial.no_access', ['message' => _mt($this->module->moduleId, 'TWiseWallet.Sorry_but_you_don_have_permission_to_access_this_page')]);
        else
            return view('Wallet.TWiseWallet.Views.Partials.transfer', $data);
    }
    ...
    function hasSufficientBalance(Request $request)
    {
        ...
        foreach ($request->input('payee.*') as $id => $payee) {
            ...
            $totalAmount += $payee['amount'] + array_sum(array_column($charges = defineFilter('PrTWiseWalletTransaction', [], 'transactionTotal', $payee), 'amount'));
        }
    ...
    function getTransactionList(TransactionServices $transactionServices, Request $request)
    {
        ...
        return view('Wallet.TWiseWallet.Views.Partials.transactionList', $data);
    }
    ...
    function getPayInOutChart()
    {
        ...
        return view('Wallet.TWiseWallet.Views.Partials.payInOutChart', $data);
    }
    ...
    function getSettings()
    {
        $data = [
            'scripts' => [],
            'walletSettings' => TWiseWallet::where('user_id', loggedId())->first(),
            'moduleId' => $this->module->moduleId
        ];

        return view('Wallet.TWiseWallet.Views.Partials.settings', $data);
    }
    ...
    function getIpWhiteList()
    {
        ...
        return view('Wallet.TWiseWallet.Views.Partials.ipWhitelist', $data);
    }
    ...
    function getPasswordChanger()
    {
        ...
        if ($data['walletSettings']->transaction_password)
            return view('Wallet.TWiseWallet.Views.Partials.passwordChange', $data);
        else
            return view('Wallet.TWiseWallet.Views.Partials.setWalletPassword', $data);
    }
    ...
    function changePassword(Request $request)
    {
        ...
        TWiseWallet::where('user_id', loggedId())->update(['transaction_password' => bcrypt($tranPasswordData['password'])]);
        $data = [
    ...
    function whitelist(Request $request)
    {
        ...
        TWiseWallet::where('user_id', loggedId())->update($data);
        $data = [
            'module_id' => $this->module->moduleId
        ];
    ...
    function renderTranPasswordView()
    {
        ...
        return view('Wallet.TWiseWallet.Views.Partials.checkTranPassword', $data);
    }
    ...
    function setTransactionPassword(Request $request)
    {
        ...
        TWiseWallet::where('user_id', loggedId())->update(['transaction_password' => bcrypt($whitelistData['password'])]);
        $data = [
            'module_id' => $this->module->moduleId
        ];
        defineAction('postChangeTransactionPasswordActions', 'change_transaction_password', $data);
    ...
    function renderLoginPasswordView()
    {
        ...
        return view('Wallet.TWiseWallet.Views.Partials.checkLoginPassword', $data);
    }
     ...
     */
    function checkTWiseWalletPassword(Request $request)
    {
        if (Gate::denies('TWiseWalletTransaction'))
            return response()->json(['status' => false, 'error' => _mt($this->module->moduleId, 'TWiseWallet.Incorrect_transaction_password')], 401);
    ...
    function getMemberTransactionList(TransactionServices $transactionServices, Request $request, $user = null)
    {
        ...
        return view('Wallet.TWiseWallet.Views.Partials.memberTransactionList', $data);
    }
