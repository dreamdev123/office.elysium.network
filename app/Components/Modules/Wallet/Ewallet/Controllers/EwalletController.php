...
namespace App\Components\Modules\Wallet\Ewallet\Controllers;
...
use App\Components\Modules\Wallet\Ewallet\EwalletIndex;
use App\Components\Modules\Wallet\Ewallet\EwalletIndex as Module;
 ...
 * @package App\Components\Modules\Wallet\Ewallet\Controllers
 */
class EwalletController extends Controller
{
    ...
    function index()
    {
        $data = [
            ...
            'wallet' => [],
            'balance' => $this->module->getBalance(loggedUser(), false),
            'moduleId' => $this->module->moduleId,
            'walletConfig' => getModule($this->module->moduleId)->getModuleData(true)
        ];

        return view('Wallet.Ewallet.Views.index', $data);
    }
    ...
    function getOverView()
    {
        ...
        return view('Wallet.Ewallet.Views.Partials.overView', $data);
    }
    ...
    function getIncomeChart()
    {
        ...
        return view('Wallet.Ewallet.Views.Partials.incomeExpenseGraph', $data);
    }
    ...
    function getVcc()
    {
        ...
        return view('Wallet.Ewallet.Views.Partials.vcc', $data);
    }
    ...
    function getTransfer(Request $request, ModuleServices $moduleServices)
    {
        $data = [
            ...
            'moduleId' => $this->module->moduleId,
            'gateway' => callModule('Payment-Ewallet')->moduleId,
            'walletConfig' => getModule($this->module->moduleId)->getModuleData(true),
        ];
        $walletData = Ewallet::where('user_id', loggedId())->first();

        if (!isset($walletData)) {
            Ewallet::create(['user_id' => loggedId()]);
            $walletData = Ewallet::where('user_id', loggedId())->first();
        }

        if (!$data['walletConfig']->get('transfer_provision'))
            return view('Common.Partial.no_access', ['message' => _mt($this->module->moduleId, 'Ewallet.Sorry_fund_transfer_has_been_disabled_in_this_wallet')]);
        else if (!checkAccess(loggedId(), 'fund_transfer'))
            return view('Common.Partial.no_access', ['message' => _mt($this->module->moduleId, 'Ewallet.Sorry_but_you_don_have_permission_to_send_fund')]);
        else if ($walletData->ip_status && $walletData->ip != $request->ip())
            return view('Common.Partial.no_access', ['message' => _mt($this->module->moduleId, 'Ewallet.Sorry_but_you_don_have_permission_to_access_this_page')]);
        else
            return view('Wallet.Ewallet.Views.Partials.transfer', $data);
    }
    ...
    {
        return $moduleServices->callModule('Payment-Ewallet')->renderView();
    }
    ...
    function getTransactionList(TransactionServices $transactionServices, Request $request)
    {
        ...
        return view('Wallet.Ewallet.Views.Partials.transactionList', $data);
    }
    ...
    function getPayInOutChart()
    {
        ...
        return view('Wallet.Ewallet.Views.Partials.payInOutChart', $data);
    }
    ...
    function getSettings()
    {
        $data = [
            'scripts' => [],
            'walletSettings' => Ewallet::where('user_id', loggedId())->first(),
            'moduleId' => $this->module->moduleId
        ];

        return view('Wallet.Ewallet.Views.Partials.settings', $data);
    }
    ...
    function getIpWhiteList()
    {
        ...
        return view('Wallet.Ewallet.Views.Partials.ipWhitelist', $data);
    }
    ...
    function getPasswordChanger()
    {
        ...
        if ($data['walletSettings']->transaction_password)
            return view('Wallet.Ewallet.Views.Partials.passwordChange', $data);
        else
            return view('Wallet.Ewallet.Views.Partials.setWalletPassword', $data);
    }
    ...
    function submitTransactionPass(Request $request)
    {
        ...
        if (!Hash::check($request->input('transactionPass'), User::find(loggedId())->password)) {
    ...
    function changePassword(Request $request)
    {
        ...
        Ewallet::where('user_id', loggedId())->update(['transaction_password' => bcrypt($tranPasswordData['password'])]);
        $data = [
    ...
    function whitelist(Request $request)
    {
        ...
        Ewallet::where('user_id', loggedId())->update($data);
        $data = [
            'module_id' => $this->module->moduleId
        ];
    ...
    function renderTranPasswordView()
    {
        ...
        return view('Wallet.Ewallet.Views.Partials.checkTranPassword', $data);
    }
    ...
    function setTransactionPassword(Request $request)
    {
        ...
        Ewallet::where('user_id', loggedId())->update(['transaction_password' => bcrypt($whitelistData['password'])]);
        $data = [
            'module_id' => $this->module->moduleId
        ];
        defineAction('postChangeTransactionPasswordActions', 'change_transaction_password', $data);
    ...
    function renderLoginPasswordView()
    {
        ...
        return view('Wallet.Ewallet.Views.Partials.checkLoginPassword', $data);
    }
     ...
     */
    function checkEwalletPassword(Request $request)
    {
        if (Gate::denies('ewalletTransaction'))
            return response()->json(['status' => false, 'error' => _mt($this->module->moduleId, 'Ewallet.Incorrect_transaction_password')], 401);
    ...
    function getMemberTransactionList(TransactionServices $transactionServices, Request $request, $user = null)
    {
        ...
        return view('Wallet.Ewallet.Views.Partials.memberTransactionList', $data);
    }
