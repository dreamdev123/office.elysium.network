<div class="wallet transactionPassWrapper">
    <div class="innerWrap">
        <div class="eachField">
            <label>{{_mt($moduleId,'Ewallet.please_enter_your_login_password') }}</label>
        </div>
        <div class="eachField">
            <input type="password" placeholder="{{_mt($moduleId,'Ewallet.enter_login_password') }}"
...
<script type="text/javascript">
    ...
    $('body').off('click', '.submitTransactionPass').on('click', '.submitTransactionPass', function () {
        ...
        $.post('{{ route(strtolower(getScope()).".ewallet.submitTransactionPass") }}', options, function (response) {
            Ladda.stopAll();
            if (window.hasOwnProperty('transactionPassSuccessCallback')) {
                window.transactionPassSuccessCallback(response, 'ewallet');
            } else {
