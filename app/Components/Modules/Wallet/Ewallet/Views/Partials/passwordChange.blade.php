<form class="passChange col-md-12">
    ...
    <div class="inputs">
        <h2 class="heading">{{ _mt($moduleId,'Ewallet.change_transaction_password') }}</h2>
        <div class="firstPart">
            <div class="changePasswordFields">
                <div class="row changePass">
                    <div class="form-group">
                        <label class="control-label col-md-4">{{ _mt($moduleId,'Ewallet.new_password') }}
                ...
                <div class="row changePass">
                    <div class="form-group">
                        <label class="control-label col-md-4">{{ _mt($moduleId,'Ewallet.re_enter_password') }}
                            <span class="required" aria-required="true"> * </span>
                        </label>
                        <div class="col-md-6">
                            <div class="input-group">
                            ...
                            <label class="control-label" style="color: red; font-size: 16px; margin-top: 8px;"> Please enter minimum 8 characters. </label>
                        </div>
                    </div>
                </div>
                <div class="row">
            ...
            <div class="successNotice">
                ...
                <h3>{{ _mt($moduleId,'Ewallet.tran_password_updated') }}</h3>
            </div>
        </div>
    </div>
</form>
...
<script type="text/javascript">
    ...
    $('button.changePassword').click(function () {
        ...
        var post = $.post('{{ route(strtolower(getScope()).".ewallet.validate.change_password") }}', formData, function (response) {
            $('.changePasswordFields').slideUp();
            refreshAjaxData($('.transactionPassLoader'), '{!! route(strtolower(getScope()).".ewallet.tran.password.view") !!}');
            Ladda.stopAll();
    ...
    function validatePasswordForm() {
        ...
        if ((password.length < 8) || (confirmPassword.length < 8)) {
            addValidationError({password: "{{ _mt($moduleId,'Ewallet.re_enter_password') }}"});
            return false;
        }
        if (password != confirmPassword) {
            addValidationError({missmatch: "{{ _mt($moduleId,'Ewallet.Confirm_Password_Miss_Match') }}"});
            return false;
        }
