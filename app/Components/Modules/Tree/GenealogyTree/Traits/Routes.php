        ...
        foreach (getScopes()->except('shared') as $scope)
            Route::group(['prefix' => $scope, 'middleware' => "Routeserver:$scope"], function () use ($scope) {
                Route::group(['prefix' => 'tree'], function () use ($scope) {
                    ...
                    Route::group(['prefix' => 'genealogyTree1'], function () use ($scope) {
                        Route::match(['get', 'post'], 'view/{type?}/{id?}', 'GenealogyTreeController@index1')->name("$scope.tree.genealogyTree1");
                        Route::match(['get', 'post'], 'reload/{id?}', 'GenealogyTreeController@reload1')->name("$scope.tree.genealogyTree1.reload");
                        Route::match(['get', 'post'], 'tooltip/{id}', 'GenealogyTreeController@tooltip1')->name("$scope.tree.genealogyTree1.tooltip");
                    });
                });

                Route::group(['prefix' => 'tree'], function () use ($scope) {
                    Route::group(['prefix' => 'userRegister','middleware'=>'auth'], function () use ($scope) {
