...
class GenealogyTreeController extends Controller
{
    ...
    {
        if (!$user->package || $user->package->slug == 'affiliate'){
            return asset('photos/Affiliate.jpeg');
        } elseif (!$user->package || $user->package->slug == 'influencer') {
            return asset('photos/influencer.png');
        } elseif (!$user->package || $user->package->slug == 'client') {
            return asset('photos/client.jpeg');
        } else {
            return asset('photos/IB.jpeg');
        }
        //return asset((!$user->package || $user->package->slug == 'affiliate') ? 'photos/femaleUser.png' : 'photos/maleUser.png');
    }
     ...
     * @throws Throwable
     */
    function index1(Request $request, $type = 'placement', $userId = null)
    {
        if (!$this->activePlan)
            dd('You cant able to access this page, Because no mlm plan found');
        ...
        $data = [
            'scripts' => [
                // $this->module->getJsPath() . 'main1.js',
                asset('global/plugins/panzoom/dist/jquery.panzoom.min.js'),
            ],
            'styles' => [
                $this->module->getCssPath() . 'hierarchy-view.css',
                // $this->module->getCssPath() . 'style1.css',
            ],
            'title' => _mt($this->module->moduleId, 'GenealogyTree.genealogy_tree'),
            'heading_text' => _mt($this->module->moduleId, 'GenealogyTree.genealogy_tree'),
            'breadcrumbs' => [
        ...
        $option['type'] = $data['type'] = $type;
        $option['parent'] = $userId;
        $data['content'] = $this->renderTree1($option);
        $data['moduleId'] = $this->module->moduleId;
        $data['moduleData'] = $this->module->getModuleData(true);
        $iconRepresentation = $data['moduleData']['tree_avatar'];
        switch ($data['moduleData']['tree_avatar']) {
        ...
        return view('Tree.GenealogyTree.Views.genealogyTree1', $data);
    }
     ...
     * @throws Throwable
     */
    function renderTree1($options)
    {
        $data = $dispatch = [];
        $dispatch['type'] = $data['type'] = $this->treeType = $options['type'];
        $dispatch['parentId'] = $options['parent'];
        $data['downLines'] = $downLines = $this->getData1(collect($dispatch));
        $data['moduleData'] = $this->module->getModuleData(true);

        if (isset($options['markupOnly']) && $options['markupOnly']) return $downLines;

        return view('Tree.GenealogyTree.Views.treeView1', $data);
    }
     ...
     * @return string
     */
    function getData1(Collection $options)
    {
        /** @var UserRepo $parent */
        $parent = UserRepo::withDepth('level')->with('user')->find($options->get('parentId'));
        $this->level += $parent->level;
     ...
     * @return string
     */
    function setHtml1(UserRepo $node, $customHtml = null, $append = false)
    {
        //set tree avatar
        switch ($this->moduleConfig->get('tree_avatar')) {
            case 'default_icon':
                        ...
                        <a href="javascript:void(0);">
                            <div class="member-view-box">
                                <div class="member-image">
                                    <img data-id="' . $node->user_id . '" src="' . $treeAvatar . '" alt="Member">
                                    <div class="member-details">
                                        <h3 class="username-style">' . $node->user->username . '</h3>
                                    </div>
                                </div>
                            </div>
                        </a>
     ...
     * @return string
     */
    function setHtml2(UserRepo $node, $customHtml = null, $append = false)
    {
        //set tree avatar
        switch ($this->moduleConfig->get('tree_avatar')) {
            case 'default_icon':
     ...
     * @return string
     */
    function setChildNode1(UserRepo $node)
    {
        $width = isset($this->activePlan->getModuleData()['width']) ? $this->activePlan->getModuleData()['width'] : 'unlimited';
        $html = '<ul class="active">';
        if ($this->activePlan->getRegistry()['key'] == 'Binary' && $node->children[0]['position'] == 2)
            $html .= ($width == 'unlimited' || count($node->children) < $width) ? $this->setEmptyNode1($node) : '';
            $html .= '';
        foreach ($node->children as $child) {
            $html .= '<li>';
     ...
     * @throws Throwable
     */
    function reload1($parent = null, Request $request)
    {
        $defaults = [
            'parent' => $parent ?: loggedId(),
            'type' => 'placement',
    ...
    function tooltip1($id = null)
    {
        ...
        $data['sponsor'] = User::where('id','=',UserRepo::where('user_id','=',$data['profile']->id)->first()->sponsor_id)->first();
        return view('Tree.GenealogyTree.Views.tooltip1', $data);
    }
     ...
     * @return string
     */
    function setHtml12(UserRepo $node, $customHtml = null, $append = false)
    {
        //set tree avatar
        switch ($this->moduleConfig->get('tree_avatar')) {
            case 'default_icon':
     ...
     * @return string
     */
    function setHtml22(UserRepo $node, $customHtml = null, $append = false)
    {
        //set tree avatar
        switch ($this->moduleConfig->get('tree_avatar')) {
            case 'default_icon':
     ...
     * @return string
     */
    function setChildNode2(UserRepo $node, $mainChild = null)
    {
        $width = isset($this->activePlan->getModuleData()['width']) ? $this->activePlan->getModuleData()['width'] : 'unlimited';
     ...
     * @return string
     */
    function setEmptyNode1(UserRepo $node)
    {
        if ($this->moduleConfig->get('tree_registration'))
            $emptyNode = '<li>
                    <div class="hv-item">' . $this->setNodeRegisterButton($node) . '</div>                        
