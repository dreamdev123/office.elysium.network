        ...
        <table id="table" class="table table-striped table-hover">
            @if(in_array('fullname', $moduleData['tooltip_info']))
                <tr>
                    ...
                    <td>{{ $profile->hide_name?'N/A':$profile->metaData->firstname . ' ' .$profile->metaData->lastname }}</td>
                </tr>
            @endif
            @if(in_array('firstname', $moduleData['tooltip_info']))
                <tr>
                    <td>{{ _mt($moduleId,'TabularTree.firstname') }}</td>
                    <td>{{ $profile->hide_name?'N/A':$profile->metaData->firstname }}</td>
                </tr>
            @endif
            @if(in_array('lastname', $moduleData['tooltip_info']))
                <tr>
                    <td>{{ _mt($moduleId,'TabularTree.lastname') }}</td>
                    <td>{{ $profile->hide_name?'N/A':$profile->metaData->lastname }}</td>
                </tr>
            @endif
