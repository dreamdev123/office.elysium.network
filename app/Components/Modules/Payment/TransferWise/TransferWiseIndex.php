...
class TransferWiseIndex extends PaymentModuleAbstract
{
    ...
    function initialize(ModuleServices $moduleServices)
    {
        $this->setService(app(PaymentServices::class))
            ->setWallet($moduleServices->callModule('Wallet-ZPayWallet'));
    }
    ...
    function processPayment(Payable $payable, TransactionServices $transactionServices, PaymentServices $paymentServices)
    {
        ...
        if($context == 'package_upgrade' || $context == 'Subscription')
        {
            $user = loggedUser();
            $datainput = ['username'=>$user->username];

        }
        $datatransfer = TransferWiseTransaction::where('reference',$datainput['username'])->where('context',$context)->first();
    ...
    function gettransfer($starttime,$endtime,$profileid)
    {
        ...
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => $url . '/v1/transfers?offset=0&limit=200&profile=' . $profileid . '&createdDateStart=' . $starttime.'.000Z' . '&createdDateEnd=' . $endtime . '.000Z',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
    ...
    {
        // Setting necessary vars in Payable
        $payable->setFromModule(Module::where('slug', 'Wallet-Ewallet')->first()->id);
        // Adding transaction record with operation details
        $transaction = $transactionServices
            ->processTransaction($payable)
            ->setOperation()
