        ...
        foreach (getScopes()->except('shared') as $scope)
            Route::group(['prefix' => $scope, 'middleware' => "Routeserver:$scope"], function () use ($scope) {
                Route::group(['prefix' => 'zota'], function () use ($scope) {
                    ...
                    if($scope == 'admin')
                    {
                        Route::get('transferwisetable','TransferWiseController@transferwisetable')->name('admin.Transferwise.table');
                        Route::get('list','TransferWiseController@list')->name('admin.Transferwise.list');
                    }
                });
            });
