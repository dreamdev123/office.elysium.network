...
class TransferWiseController extends Controller
{
    ...
    function actionuser(Request $request)
    {
        $username = $request->input('username');
    ...
    //        $address = '5e2697a10ee97fc79250f8c5b804390e8da280b4cf06e';
            $transferwise = TransferWiseTransaction::where('reference', $username)->where('status',0)->latest()->first();
            ...
            if ($transferwise && $transferwise->status == 0) {
                ...
                if($transferwise->context == 'Registration')
                {
                    $callback = new $transferwise->callback;
                ...
                if($user)
                {
                    if($transferwise->context == 'package_upgrade')
                    {
                        $extenalmailservice = new ExternalMailServices;
                        app()->call([$extenalmailservice,'upgradesendmail'],['user'=>$user,'gateway'=>'transferwise','data'=>array('amount'=>$transferwise->amount)]);    
                    }
            ...
            {
                $user = User::where("username",$username)->first();
                $transferwise = TransferWiseTransaction::where('reference',$user->customer_id)->latest()->first();
            }
        });
        $transferwise = TransferWiseTransaction::where('reference', $username)->latest()->first();
        $user = User::where("username",$username)->first();
        if(!$transferwise)
        {
            $transferwise = TransferWiseTransaction::where('reference',$user->customer_id)->latest()->first();
        ...
        if($transferwise)
        {
            return array('amount'=>$transferwise->amount,'user'=>$user,'context'=>$transferwise->context);    
        }
    ...
    function transferwisetable(Request $request)
    {
