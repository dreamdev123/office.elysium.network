...
class AdvancedRankIndex extends BasicContract implements RankModuleInterface
{
    ...
    {
        //return $this->isQualified(AdvancedRank::find(7), $user);
    ...
    function isQualified(AdvancedRank $rank, User $user)
    {
        $checkChildren = function ($ranksToCheck) use ($user, $rank) {
            ...
            if (!$ranksToCheck['rank'] || !$ranksToCheck['count']) return true;

            if (!$user->repoData) return false;
            /** @var Collection $children */
            $children = $user->repoData->applySponsorHierarchy()->children()->get()->filter(function ($child) use ($ranksToCheck, $checkMaintenance, $rank) {
                /** @var User $child */
                return ($child->rank && ($child->rank->rank_id >= $ranksToCheck['rank']) && ($checkMaintenance ? $child->user->expiry_date >= Carbon::now() : true));
            });

            if ($children->count() < $ranksToCheck['count']) return false;

            if ($children->count() && ($minLegs = $ranksToCheck['minCount'])) {
                $leftReferrals = $user->repoData->leftReferrals(true)->whereIn('user_id', $children->pluck('user_id')->toArray())->count();
                $rightReferrals = $user->repoData->rightReferrals(true)->whereIn('user_id', $children->pluck('user_id')->toArray())->count();
        ...
        if ($user->package && ($user->package->slug == 'affiliate' || $user->package->slug == 'client')) return false;
     ...
     * @return mixed|void
     */
    function calculateUpLineRank(User $user)
    {
        $user = \Auth::getUser($user->id);
        $users = (new UserRepo)->ancestorsOfQuery($user->id, 'placement')->get();
        $users->each(function ($user) {
            $rank = $this->calculate($user);
            if ($rank) $this->distribute($user, $rank);
        });

        $users = (new UserRepo)->ancestorsOfQuery($user->id, 'sponsor')->get();
        $users->each(function ($user) {
            $rank = $this->calculate($user);
            if ($rank) $this->distribute($user, $rank);
        });
