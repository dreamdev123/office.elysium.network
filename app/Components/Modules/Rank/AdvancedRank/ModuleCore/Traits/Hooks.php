...
trait Hooks
{
    ...
    function hooks()
    {
        ...
        registerAction('postRegistration', function ($data) {
            if ($data->get('result')['user']->package && $data->get('result')['user']->package->slug != 'affiliate' && $data->get('result')['user']->package->slug != 'client') $this->distribute($data->get('result')['user'], AdvancedRank::find(1));
        }, 'registration', 0);

        registerAction('postAddToRepo', function ($data) {
            $user = $data['user'];
            if ($user->package && $user->package->slug != 'affiliate' && $user->package->slug != 'client') $this->distribute($data->get('result')['user'], AdvancedRank::find(1));
        }, 'holdingTank', 0);
    }
