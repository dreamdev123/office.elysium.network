...
class BinaryPointReportController extends Controller
{
    ...
    function fetch(Request $request, BinaryServices $binaryServices)
    {
        ...
        if (key_exists('dates', $filters) && $filters['dates'] != null) {
        ...
        $data['overView'] = $binaryServices->getPoints($options->except(['fromDate', 'toDate', 'isCredit'])->merge(['fullStats' => true]))->first();
        $data['overView']->cycle = $binaryServices->getPairCount(['user' => $options['userId'], 'fromDate' => $defaults['fromDate'], 'toDate' => $defaults['toDate']]);
        $data['moduleId'] = $this->module->moduleId;
