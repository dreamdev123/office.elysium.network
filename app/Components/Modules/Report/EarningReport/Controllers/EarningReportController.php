...
class EarningReportController extends Controller
{
    ...
    function getunpaiduser()
    {
        $transferwise = TransferWiseTransaction::all();
        $b2btransaction = B2BPayTransaction::all();

        $customerid = array();

        foreach ($transferwise as $key => $item) {
            if($item->status)
            {
                array_push($customerid,$item->reference);    
            }
            
        }

        foreach($b2btransaction as $key => $item)
        {
            if($item->status)
            {
                array_push($customerid,$item->reference_id);
            }
        }
    ...
    function calculateearning(Request $request)
    {
        $filters = $request->input('filters');
        if ((getScope() !== 'admin') && (getScope() !== 'employee')) {
            $filters['user_id'] = loggedId();
        }
        $earnings = app()->call([$this, 'fetchEarnings'], ['filters' => collect($filters), 'pages' => $request->input('totalToShow', 10)]); 

        $sum = 0;
        foreach ($earnings as $key => $earning) {
            $sum += $earning->transaction->actual_amount;
        }
    ...
    function fetchEarnings(Collection $filters, TransactionServices $transactionServices, $pages = null, $showAll = false)
    {
        ...
        $earnings = Commission::with(['transaction.payerUser', 'transaction.payeeUser', 'transaction.operation'])
            ...
            ->whereHas('referenceUser',function($query) use ($customerid){
                if(count($customerid) > 0)
                {
                    $query->whereNotIn('customer_id',$customerid);
                }
            })
