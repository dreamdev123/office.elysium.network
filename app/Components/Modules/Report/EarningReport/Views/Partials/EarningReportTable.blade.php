    ...
    <table class="table table-striped table-bordered table-hover dataTable dtr-inline reporttable" role="grid"
        ...
        @foreach($earnings as $earning)
            ...
            <tr>
                ...
                <td> {{ getModule($earning->module_id)?getModule($earning->module_id)->getRegistry()['name']:'' }}

                    @if(getModule($earning->module_id) && ((getModule($earning->module_id)->registry['slug'] === 'Commission-DirectReferralBonus') || (getModule($earning->module_id)->registry['slug'] === 'Commission-IndirectReferralBonus')))
                        <br>
                        <span>{{ _mt($moduleId, 'EarningReport.from') }}  {{ usernameFromId($earning->ref_user_id) }}  ({{ getUser($earning->ref_user_id)->email }})</span> @endif


                </td>
                <td> {{getModule($earning->transaction->operation->to_module) &&  getModule($earning->transaction->operation->to_module)->getRegistry()['name'] }} </td>
                @if(getScope() === 'admin')
