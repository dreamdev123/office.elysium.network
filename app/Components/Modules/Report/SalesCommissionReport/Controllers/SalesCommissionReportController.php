...
namespace App\Components\Modules\Report\SalesCommissionReport\Controllers;
ini_set('max_execution_time', 3200);
set_time_limit(3200);
use App\Blueprint\Query\Builder;
use App\Blueprint\Services\OrderServices;
use App\Components\Modules\Report\SalesCommissionReport\SalesCommissionReportIndex as Module;
...
/**
 * Class SalesCommissionReportController
 * @package App\Components\Modules\Report\SalesCommissionReport\Controllers
 */
class SalesCommissionReportController extends Controller
{
    ...
    function index()
    {
        ...
        $data['title'] = _mt($this->module->moduleId, 'SalesCommissionReport.sales_commission_report');
        $data['heading_text'] = _mt($this->module->moduleId, 'SalesCommissionReport.sales_commission_report');
        // $data['breadcrumbs'] = [
        //     _t('index.home') => 'admin.home',
        //     _mt($this->module->moduleId, 'SalesCommissionReport.Report') => 'report.sales',
        //     _mt($this->module->moduleId, 'SalesCommissionReport.sales_commission_report') => 'report.sales'
        // ];

        return view('Report.SalesCommissionReport.Views.salesCommissionReportIndex', $data);
    }
    ...
    function filters()
    {
        ...
        return view('Report.SalesCommissionReport.Views.Partials.filter', $data);
    }
     ...
     * @return Factory|View
     */
    function fetch(Request $request, OrderServices $orderServices, PayoutServices $payoutServices)
    {

        $filters = $request->input('filters');
        $data['salesData'] = $salesData = app()->call([$this, 'fetchSalesData'], ['filters' => collect($filters), 'pages' => $request->input('totalToShow', 10)]);
        $data['moduleId'] = $this->module->moduleId;
        $commission_status = [];
        foreach ($salesData as $sale) {
            $commission_status_array = [];
            ...
            foreach ($statuses as $status)
                $commission_status_array[$status] = $requestDetails->filter(function ($value) use ($status) {
                    return PayoutStatus::getSlugFromId($value->status) == $status;
                })->first();

            $eos_wallet_id = \App\Eloquents\Module::where('slug', 'Wallet-Ewallet')->first()->id;
            $walletModule = getModule((int)$eos_wallet_id);
            $commission_status_array['balance'] = $payoutServices->balanceByWallet(User::find($sale->user_id), $walletModule) - Transaction::where('context', 'commission')->where('payee', $sale->user_id)->where('status', 0)->sum('amount');

            array_push($commission_status, $commission_status_array);
        }
        $data['commission_status'] = $commission_status;

        return view('Report.SalesCommissionReport.Views.Partials.salesCommissionReportTable', $data);
    }
    ...
    function printSalesReport(Request $request)
    {
        ...
        $data = [
            'salesData' => app()->call([$this, 'fetchSalesData'], ['filters' => collect($filters), 'showAll' => true]),
            'moduleId' => $this->module->moduleId,
            'reportName' => _mt($this->module->moduleId, 'SalesCommissionReport.sales_report'),
            'displayLogo' => true
        ];

        return view('Report.SalesCommissionReport.Views.Partials.exportView', $data);
    }
}
