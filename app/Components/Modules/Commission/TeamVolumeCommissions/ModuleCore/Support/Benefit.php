...
namespace App\Components\Modules\Commission\TeamVolumeCommissions\ModuleCore\Support;
...
/**
 * Class Benefit
 * @package App\Components\Modules\Commission\TeamVolumeCommissions\ModuleCore\Support
 */
    ...
    function calculateBenefit($data = [])
    {
        ...
        if (($pairCeilingType = $data['config']->get('pair_ceiling_type')) != 'none') {
            ...
            if ($data['config']->get('pair_ceiling_based') == 'pair_count') {
                ...
                $pairAmount = callModule('Commission-TeamVolumeCommissions')->credited($data['beneficiary'], $options)->sum('amount');
    ...
    function distribute()
    {
        DB::transaction(function () {
            ...
            foreach ($this->getBeneficiaries() as $beneficiary) {
                ...
                callmodule('Commission-GenerationalMatchingBonus', 'process', ['data' => ['user' => $beneficiary['user'], 'amount' => $beneficiary['benefit']['benefit']]]);
                /* $founderPakcks = [3, 5];
