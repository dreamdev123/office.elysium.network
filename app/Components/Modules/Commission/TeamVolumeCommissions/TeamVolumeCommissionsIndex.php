...
namespace App\Components\Modules\Commission\TeamVolumeCommissions;
...
use App\Components\Modules\Commission\TeamVolumeCommissions\ModuleCore\Support\Benefit;
use App\Components\Modules\Commission\TeamVolumeCommissions\ModuleCore\Traits\Hooks;
use App\Components\Modules\Commission\TeamVolumeCommissions\ModuleCore\Traits\Validations;
...
/**
 * Class TeamVolumeCommissionsIndex
 * @package App\Components\Modules\Commission\TeamVolumeCommissions
 */
class TeamVolumeCommissionsIndex extends BasicContract implements CommissionModuleInterface
{
    ...
    function bootMethods(CommissionServices $commissionServices)
    {
        ...
        schedule('Distribute Pending TVC', function () {
    ...
    function distributePending()
    {
        ...
        $term = 'Investment';
        $transationDetails = TransactionDetail::where('to_module', 0)
            ->where(function($query) use ($term) { // $term is the search term on the query string
               $query->where('remarks', 'commission')
               ->orWhere('remarks', 'LIKE', '%' . $term . '%');
            })->get();

        $transationDetails->each(function ($transationDetail) {
            TransactionDetail::where('id', $transationDetail->id)->update(['to_module' => 7]);
        });
    }
    ...
    function adminConfig($data = [])
    {
        ...
        return view('Commission.TeamVolumeCommissions.Views.adminConfig', $data);
    }
    ...
    /**
     * @param mixed $service
     * @return TeamVolumeCommissionsIndex
     */
    ...
    function commissionTable($commissions)
    {
        ...
        return view('Commission.TeamVolumeCommissions.Views.commissionTable', $data);
    }

    function deleteAlreadyDistributed()
    {
        DB::transaction(function () {
            $tvcGmpCommissions = Commission::where('module_id', getModule('Commission-TeamVolumeCommissions')->moduleId)
                ->orWhere('module_id', getModule('Commission-GenerationalMatchingBonus')->moduleId)->get();
            $tvcGmpCommissions->each(function ($commission) {
