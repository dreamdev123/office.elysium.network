...
class Benefit extends CommissionManager
{
    ...
    {
        $start = Carbon::now()->subDay(8);
        $end = Carbon::now()->subDay(2);
        // return InvestmentRoi::with('client')->whereBetween('created_at', [$start, $end])->get();
        return InvestmentRoi::with('client')->where('profit', '>', 0)->get();
    }
    ...
    function distribute()
    {
        ...
        DB::transaction(function () use ($plugins, $config) {
            foreach ($this->getBeneficiaries() as $roi) {
                ...
                $uplineUsers = UserRepo::getAncestorsOf($sponsorId, 'sponsor', count($config->get('commission'))-1);
                ...
                $uplineUsers->each(function ($repo, $key) use ($plugins, $config, $roi, &$data) {
                    ...
                    /** @var Plugins $plugins */
                    if (!(double)$benefit['benefit'] || !checkAccess($user->id, 'commission') || ($plugins->isAffiliate($user) && $key != 0)) return;
    ...
    function calculateBenefit($data = [])
    {
        ...
        return [
            ...
            'benefit' => $amount,
            'distributeAmount' => $distributionAmount
        ];
    }
}