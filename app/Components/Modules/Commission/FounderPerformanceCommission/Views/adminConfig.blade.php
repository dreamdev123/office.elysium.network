...
@endforeach
<div class="row moduleConfig FounderPerformanceCommission">
    <div class="col-sm-12">
        ...
        <div class="panel panel-primary">
            <div class="panel-heading"><span aria-hidden="true" class="icon-tag"></span>
                Founder Performance Commission
            </div>
            <div class="panel-body">
                <div class="row form-group">
                    <div class="col-md-3">
                        <label> {{ _mt($moduleId,'FounderPerformanceCommission.To_Wallet') }}</label>
                    </div>
                ...
                <div class="row form-group">
                    ...
                    <div class="col-md-9">
                        <input type="text" name="module_data[rate]" class="form-control input-medium"
                               value="{{ $config->get('rate') }}">
                    </div>
                </div>
                <div class="form-group offset4">
                    <div class="col-md-1">
                        <button type="button" value="{{ _mt($moduleId,'FounderPerformanceCommission.Save') }}"
                                class="form-control ladda-button btn green button-submit" data-style="contract"
                                name="amount">{{ _mt($moduleId,'FounderPerformanceCommission.Save') }}</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    $(function () {
        ...
        var validator = form.validate({
            ...
            messages: {
                'module_data[wallet]': "{{ _mt($moduleId,'FounderPerformanceCommission.Please_select_wallet') }}",
                'module_data[rate]': "Please enter rate",
            },
        ...
        $('.FounderPerformanceCommission .button-submit').click(function () {

            $('.ajaxValidationError').removeClass('ajaxValidationError');

            var successCallBack = function (response) {
                Ladda.stopAll();
                toastr.success("{{ _mt($moduleId,'FounderPerformanceCommission.Configuration_saved_successfully') }}");
            };
