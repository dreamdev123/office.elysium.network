...
class Benefit extends CommissionManager
{
    ...
    function distribute()
    {
        ...
        DB::transaction(function () use ($plugins, $benefit, $wallet, $config) {
            foreach ($this->getBeneficiaries() as $rankHolder) {
                $beneficiary = [
                    'user' => $rankHolder->user,
                    'benefit' => [
                        'commissionType' => PaymentCommission::class,
                        'wallet' => $plugins->isPaidUser($rankHolder->user) ? $wallet->getRegistry()['moduleId'] : getModule('Wallet-PendingWallet')->moduleId,
                        'benefit' => $benefit
                    ],
                    'credit_status' => 1
                ];
