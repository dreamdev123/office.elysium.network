...
@endforeach
<div class="row moduleConfig GenerationalMatchingBonus">
    <div class="col-sm-12">
        {!! Form::open(['route' => ['admin.module.configure',$moduleId],'class' => 'form-horizontal adminConfig','id' => 'adminConfig']) !!}
        <div class="panel panel-primary">
            <div class="panel-heading">
                <span aria-hidden="true"
                      class="icon-tag"></span> Generation Matching Bonus [GMP]
            </div>
            <div class="panel-body">
                <div class="row form-group">
                    <div class="col-md-3">
                        <label> {{ _mt($moduleId,'GenerationalMatchingBonus.To_Wallet') }}</label>
                    </div>
                    ...
                    <div class="col-md-4 col-xs-4">
                        <div class="col-md-5">
                            <input type="button" id="addLevel" class="btn btn-sm green"
                                   value="{{ _mt($moduleId,'GenerationalMatchingBonus.Add_Level') }}"/>
                        </div>
                        <div class="col-md-5">
                            <input type="button" id="removeLevel" class="btn btn-sm red"
                                   value="{{ _mt($moduleId,'GenerationalMatchingBonus.Remove_Level') }}"/>
                        </div>
                    </div>
                </div>
                <div class="form-group offset4">
                    <div class="col-md-1">
                        <button type="button" value="{{ _mt($moduleId,'GenerationalMatchingBonus.Save') }}"
                                class="form-control ladda-button btn green button-submit" data-style="contract"
                                name="amount">{{ _mt($moduleId,'GenerationalMatchingBonus.Save') }}</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    $(function () {
        ...
        var validator = form.validate({
            ...
            messages: {
                'module_data[distribution_type]': "{{ _mt($moduleId,'GenerationalMatchingBonus.Please_choose_distribution_type') }}",
                'module_data[wallet]': "{{ _mt($moduleId,'GenerationalMatchingBonus.Please_select_wallet') }}",
            },
        ...
        $('.GenerationalMatchingBonus .button-submit').click(function () {

            $('.ajaxValidationError').removeClass('ajaxValidationError');

            var successCallBack = function (response) {
                Ladda.stopAll();
                toastr.success("{{ _mt($moduleId,'GenerationalMatchingBonus.Configuration_saved_sucessfully') }}");
            };
