...
@endforeach
<div class="row moduleConfig GlobalBonusPool">
    <div class="col-sm-12">
        ...
        <div class="panel panel-primary">
            <div class="panel-heading"><span aria-hidden="true" class="icon-tag"></span>
                Diamond Bonus Pool [DBP]
            </div>
            <div class="panel-body">
                <div class="row form-group">
                    <div class="col-md-3">
                        <label> {{ _mt($moduleId,'GlobalBonusPool.To_Wallet') }}</label>
                    </div>
                ...
                <div class="row form-group">
                    <div class="col-md-3">
                        <label>
                            {{ _mt($moduleId, 'GlobalBonusPool.Pool_Share') }} <span class="type"></span>
                        </label>
                    </div>
                    <div class="col-md-6 packagePriceBox">
                        @foreach($ranks as $rank)
                            <div class="row">
                                ...
                                <div class="col-sm-7">
                                    <input type="number" name="module_data[pool_share][{{ $rank->id }}]"
                                           class="form-control"
                                           value="{{ collect($config->get('pool_share'))->get($rank->id) }}">
                                </div>
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="form-group offset4">
                    <div class="col-md-1">
                        <button type="button" value="{{ _mt($moduleId,'GlobalBonusPool.Save') }}"
                                class="form-control ladda-button btn green button-submit" data-style="contract"
                                name="amount">{{ _mt($moduleId,'GlobalBonusPool.Save') }}</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    $(function () {
        ...
        var validator = form.validate({
            ...
            rules: {
                ...
                'module_data[pool_share][]': {
                    required: true,
                    ajaxValidate: true,
                },
            },
            messages: {
                'module_data[wallet]': "{{ _mt($moduleId,'GlobalBonusPool.Please_select_wallet') }}",
                'module_data[pool_share]': "{{ _mt($moduleId,'GlobalBonusPool.Please_enter_pool_share') }}",
            },
        ...
        $('.GlobalBonusPool .button-submit').click(function () {

            $('.ajaxValidationError').removeClass('ajaxValidationError');

            var successCallBack = function (response) {
                Ladda.stopAll();
                toastr.success("{{ _mt($moduleId,'GlobalBonusPool.Configuration_saved_successfully') }}");
            };
