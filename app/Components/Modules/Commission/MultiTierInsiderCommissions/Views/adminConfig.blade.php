...
@endforeach
<div class="row moduleConfig MultiTierInsiderCommissions">
    <div class="col-sm-12">
        {!! Form::open(['route' => ['admin.module.configure',$moduleId],'class' => 'form-horizontal adminConfig','id' => 'adminConfig']) !!}
        <div class="panel panel-primary">
            <div class="panel-heading">
                <span aria-hidden="true"
                      class="icon-tag"></span> Multi-Tier EOS Commissions [MEC]
            </div>
            <div class="panel-body">
                <div class="row form-group">
                    <div class="col-md-3">
                        <label> {{ _mt($moduleId,'MultiTierInsiderCommissions.To_Wallet') }}</label>
                    </div>
                ...
                <h4><b>Monthly Subscription</b> from</h4>
                <div class="row form-group">
                    <div class="col-sm-3">
                        <label class="control-label">Tier1</label>
                    </div>
                    <div class="col-sm-3">
                        <input type="number" class="form-control" name="module_data[registration][1]"
                               value="{{ collect($config->get('registration'))->get(1) }}">
                    </div>
                </div>
                <br/>

                <h4><b>Annual Subscription</b> from</h4>
                <div class="row form-group">
                    ...
                    <div class="col-md-4 col-xs-4">
                        <div class="col-md-5">
                            <input type="button" id="addLevel" class="btn btn-sm green"
                                   value="{{ _mt($moduleId,'MultiTierInsiderCommissions.Add_Level') }}"/>
                        </div>
                        <div class="col-md-5">
                            <input type="button" id="removeLevel" class="btn btn-sm red"
                                   value="{{ _mt($moduleId,'MultiTierInsiderCommissions.Remove_Level') }}"/>
                        </div>
                    </div>
                </div>
                <div class="form-group offset4">
                    <div class="col-md-1">
                        <button type="button" value="{{ _mt($moduleId,'MultiTierInsiderCommissions.Save') }}"
                                class="form-control ladda-button btn green button-submit" data-style="contract"
                                name="amount">{{ _mt($moduleId,'MultiTierInsiderCommissions.Save') }}</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    $(function () {
        ...
        var validator = form.validate({
            ...
            messages: {
                'module_data[registration][]': {
                    required: "{{ _mt($moduleId,'MultiTierInsiderCommissions.Please_enter_registration_amount') }}",
                    number: "{{ _mt($moduleId,'MultiTierInsiderCommissions.Invalid_registration_amount') }}",
                },
                'module_data[distribution_type]': "{{ _mt($moduleId,'MultiTierInsiderCommissions.Please_choose_distribution_type') }}",
                'module_data[wallet]': "{{ _mt($moduleId,'MultiTierInsiderCommissions.Please_select_wallet') }}",
            },
        ...
        $('.MultiTierInsiderCommissions .button-submit').click(function () {

            $('.ajaxValidationError').removeClass('ajaxValidationError');

            var successCallBack = function (response) {
                Ladda.stopAll();
                toastr.success("{{ _mt($moduleId,'MultiTierInsiderCommissions.Configuration_saved_sucessfully') }}");
            };
