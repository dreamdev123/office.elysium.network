...
@endforeach
<div class="row moduleConfig FastTrackCommissions">
    <div class="col-sm-12">
        {!! Form::open(['route' => ['admin.module.configure',$moduleId],'class' => 'form-horizontal adminConfig','id' => 'adminConfig']) !!}
        <div class="panel panel-primary">
            <div class="panel-heading">
                <span aria-hidden="true"
                      class="icon-tag"></span> Fast-Track Commissions [FTC]
            </div>
            <div class="panel-body">
                <div class="row form-group">
                    <div class="col-md-3">
                        <label>{{ _mt($moduleId,'FastTrackCommissions.To_Wallet') }}</label>
                    </div>
                    <div class="col-md-3">
                ...
                <div class="row form-group">
                    <div class="col-sm-3">
                        <label class="control-label">Amount1</label>
                    </div>
                    <div class="col-sm-3">
                        <input type="number" class="form-control" name="module_data[amount1]"
                               value="{{ $config->get('amount1') }}">
                    </div>
                </div>
                <div class="row form-group">
                    <div class="col-sm-3">
                        <label class="control-label">Amount2</label>
                    </div>
                    <div class="col-sm-3">
                        <input type="number" class="form-control" name="module_data[amount2]"
                               value="{{ $config->get('amount2') }}">
                    </div>
                </div>
                <div class="row form-group">
                    <div class="col-sm-offset-3 col-md-3">
                        <button type="button" value="{{ _mt($moduleId,'FastTrackCommissions.Save') }}"
                                class="form-control ladda-button btn green button-submit" data-style="contract"
                                name="amount">{{ _mt($moduleId,'FastTrackCommissions.Save') }}</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    $(function () {
        ...
        var validator = form.validate({
            ...
            messages: {
                ...
                'module_data[wallet]': "{{ _mt($moduleId,'FastTrackCommissions.Please_select_wallet') }}",
                'module_data[amount]': {
        ...
        $('.FastTrackCommissions .button-submit').click(function () {

            $('.ajaxValidationError').removeClass('ajaxValidationError');

            var successCallBack = function (response) {
                Ladda.stopAll();
                toastr.success("{{ _mt($moduleId,'FastTrackCommissions.Configuration_saved_sucessfully') }}");
            };
