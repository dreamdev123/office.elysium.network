...
@section('content')
	...
	<div class="content-creator-page">
		...
		<div class="newSoho-custom-container">
			<div class="file-upload-dropzone-custom-form">
				...
				<form action="{{ route('newsoho.uploadImage') }}" method="POST" enctype="multipart/form-data">
				    @csrf
				...
				@if (isset($networkFileNames) && count($networkFileNames) > 0)
					@foreach($networkFileNames as $filename)
						...
						<div class="col-lg-2 col-md-3 col-sm-4 col-6 img-preview-thumbnail">
							<img src="{{ asset('uploads') }}/{{ $key }}/network/{{ $name }}">
							<i class="fa fa-trash-o remove-sohoimage" data-brand="network" data-location="{{ $key }}" data-filename="{{ $name }}"></i>
						</div>
						@endforeach
					@endforeach
				@endif
			...
			<div class="row">
				@if (isset($capitalFileNames) && count($capitalFileNames) > 0)
					@foreach($capitalFileNames as $filename)
						@foreach($filename as $key=>$name)
						<div class="col-lg-2 col-md-3 col-sm-4 col-6 img-preview-thumbnail">
							<img src="{{ asset('uploads') }}/{{ $key }}/capital/{{ $name }}">
							<i class="fa fa-trash-o remove-sohoimage" data-brand="capital" data-location="{{ $key }}" data-filename="{{ $name }}"></i>
						</div>
						@endforeach
					@endforeach
				@endif
			...
			<div class="row">
				@if (isset($insiderFileNames) && count($insiderFileNames) > 0)
					@foreach($insiderFileNames as $filename)
						@foreach($filename as $key=>$name)
						<div class="col-lg-2 col-md-3 col-sm-4 col-6 img-preview-thumbnail">
							<img src="{{ asset('uploads') }}/{{ $key }}/insider/{{ $name }}">
							<i class="fa fa-trash-o remove-sohoimage" data-brand="insider" data-location="{{ $key }}" data-filename="{{ $name }}"></i>
						</div>
						@endforeach
					@endforeach
				@endif
    ...
    $('.location-group').change(function(){
      if( $(this).is(":checked") ) {
        ...
        let send_data = {};
	      ...
	      $.ajax({
	        ...
	        data: send_data,
	        success: function(data) {
	          window.location.href = '{{ route('newsoho.imageManagement') }}?location=' + val;
	        },
	        error: function(data){
	          console.log(data);
	        }
	  ...
	  $('.remove-sohoimage').click(function() {
	  	...
	  	window.location.href = '{{ route('newsoho.removeFile') }}?type=image&brand='+brand+'&location='+location+'&filename='+filename;
	  })
	</script>
