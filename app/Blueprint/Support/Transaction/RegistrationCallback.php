...
class RegistrationCallback extends Callback
{
     ...
     * @return mixed
     */
    function success(OrderServices $orderServices, UserServices $userServices, $response, ExternalMailServices $externalMailServices, Plugins $plugins)
    {
        ...
        if($transaction->context == 'Registration')
        {
            ...
            //var_dump($user);exit;
            //$externalMailServices->sendpaymentconfirm($user,$order);
            
            $externalMailServices->send_new_user($user,$transaction);
           ...
           // }
            ...
            return tap(['transaction' => $transaction, 'orderId' => $order->id, 'user' => $user, 'redirectUrl' => $redirectUrl . '?registered=true&orderId=' . $order->id], function ($data) use ($plugins, $externalMailServices,$transaction) {
                defineAction('postAddUser', 'registration', collect(['result' => $data]));

                $module = Module::find($transaction->gateway);
                if($module->slug != 'Payment-B2BPay' && $module->slug != 'Payment-TransferWise' && $data['user']->repoData){
    ...
    function sendmailtoenroller(Collection $data, $transaction = null, $order=null, $id, $isRecepeit=false, $isRegister=false, $isUpgrade=false){
        ...
        $tag = "ENROLLER_AFFILIATE_TO_INTRODUCING BROKER - IB";
